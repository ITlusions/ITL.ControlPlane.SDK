...
```mermaid
flowchart TD
    HTTP["HTTP Request (POST /resource)"]
    API["API Server"]
    JQ["RabbitMQ Job Queue<br>provider.jobs"]
    W["Worker Pods (Worker1, Worker2, Worker3)<br>consume_jobs() → process_job() → execute_provider()"]
    P["Provider Operations (parallel)"]
    RQ["RabbitMQ Result Queue<br>provider.results"]
    HTTP --> API
    API -->|submit_job| JQ
    JQ --> W
    W --> P
    P -->|publish_result| RQ
    RQ -->|"get_job_result() / polling"| API
```
