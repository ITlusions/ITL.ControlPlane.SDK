...
```mermaid
flowchart TD
    A["HTTP Request<br>POST /resources"] --> B["API Server<br>main.py"]
    B --> C["OffloadingProviderRegistry<br>.create_or_update_resource()"]
    C --> D["JobQueue.submit_job()<br>→ RabbitMQ"]
    D --> E["Immediate Response<br>with job_id"]
    D --> Q["RabbitMQ<br>provider.jobs queue"]
    Q --> W1["Worker Pod #1"]
    Q --> W2["Worker Pod #2"]
    Q --> W3["Worker Pod #3"]
    W1 & W2 & W3 -->|consume_jobs| P["ProviderWorker<br>.process_job()"]
    P --> PA["Provider<br>.create_or_update_resource()"]
    PA --> R["Result → RabbitMQ<br>provider.results queue"]
    R --> CL["Client polls job_id<br>endpoint to get result"]
```
