...
```
        ...
        ┌───────────────┼───────────────┐
        │               │               │
    Push/PR         Manual Run       Version Tag
        │               │               │
        ↓               ↓               ↓
    CI Tests       Full Pipeline   Release Build
    (Fast)         (Complete)      (Publish)
        │               │               │
        └───────────────┼───────────────┘
                        ↓
                   Test Stage
            (Python 3.8-3.12, deps)
                        ↓
                   Security Stage
            (bandit, safety, pip-audit)
                        ↓
                   Build Stage
            (Create packages, validate)
                        ↓
            ┌───────────┴───────────┐
            │                       │
        Test PyPI        Production PyPI + Release
        (All builds)     (Version tags only)
```
