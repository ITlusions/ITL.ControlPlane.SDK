...
This is the 'B' in Option B â€” PostgreSQL remains the primary store,
Neo4j is the graph visualization layer.

Usage::

    from itl_controlplane_sdk.persistence.neo4j_sync import Neo4jSyncService
    ...
    # Used by repositories â€” they call sync automatically when configured.
"""
...
# Table name â†’ Neo4j label mapping
_TABLE_TO_LABEL = {
...
class Neo4jSyncService:
    ...
    The service is **fire-and-forget** by default â€” sync failures are
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "neo4j driver not installed â€” Neo4j sync disabled. "
                "Install with: pip install neo4j>=5.0.0"
            )
            return False
        except Exception as e:
            logger.warning("Neo4j sync connection failed: %s â€” sync disabled", e)
            return False
    ...
    async def sync_node(
        ...
        """
        Sync a resource node to Neo4j (MERGE â€” upsert).
