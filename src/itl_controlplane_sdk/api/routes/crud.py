...
Example::
    ...
    from itl_controlplane_sdk.api.routes.crud import create_crud_routes
    ...
    *,
    ...
    async def list_resources():
        try:
            ...
            response = await provider.list_resources(list_request)
            resources = [
                {
                    "id": r.id,
                    "name": r.name,
    ...
    async def get_resource(resource_name: str):
        try:
            ...
            response = await provider.get_resource(get_request)
    ...
    async def delete_resource(resource_name: str):
        try:
            ...
            response = await provider.delete_resource(delete_request)
            if response.properties and response.properties.get("error"):
                raise HTTPException(
                    status_code=404, detail=response.properties["error"]
                )
